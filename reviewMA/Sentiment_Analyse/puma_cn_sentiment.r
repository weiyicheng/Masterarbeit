...
pdf("puma_cn_sentiment.pdf")
# setwd("F:/FP_Data_Mining/R")
sink("puma_cn_sentiment.txt", append=TRUE)
...
Daten <-read.csv("puma_cn.csv", header=TRUE,fileEncoding = "UTF-8", sep = ";")
n=dim(Daten)[1]
...
# Write CSV in R
write.csv(x, file = "pumaErgebnis_cn.csv", na="")
h<-hist(dataSet$Scores, breaks=nrow(x),col="gray",main="Die Ergebnisse von chinesischen OCRs für Puma in Frequenz", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Frequenz")
...
hist(dataSet$Scores, breaks=nrow(x),freq=FALSE,col="gray",main="Die Ergebnisse von chinesischen OCRs für Puma in Dichte", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Dichte")
