...
pdf("nike_de_sentiment.pdf")
# setwd("F:/FP_Data_Mining/R")
sink("nike_de_sentiment.txt", append=TRUE)
...
Daten <-read.csv("nike_de.csv", header=TRUE,fileEncoding = "UTF-8", sep = ",")
n=dim(Daten)[1]
...
# Write CSV in R
write.csv(x, file = "nikeErgebnis_de.csv", na="")
h<-hist(dataSet$Scores, breaks=nrow(x),col="gray",main="Die Ergebnisse von deutschen OCRs für Nike in Frequenz", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Frequenz")
...
hist(dataSet$Scores, breaks=nrow(x),freq=FALSE,col="gray",main="Die Ergebnisse von deutschen OCRs für Nike in Dichte", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Dichte")
