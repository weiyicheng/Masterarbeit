...
setwd("/home/wei/Dropbox/Master Arbeit/reviewMA/Sentiment_Analyse/Marken") 
pdf("adidas2_de_sentiment.pdf")
# setwd("F:/FP_Data_Mining/R")
sink("adidas2_de_sentiment.txt", append=TRUE)
...
Daten <-read.csv("adidas2_de.csv", header=TRUE,fileEncoding = "UTF-8", sep = ",")
n=dim(Daten)[1]
...
# Write CSV in R
write.csv(x, file = "adidas2Ergebnis_de.csv", na="")
h<-hist(dataSet$Scores, breaks=nrow(x),col="gray",main="Die Ergebnisse von deutschen OCRs für Adidas weiblich in Frequenz", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Frequenz")
...
hist(dataSet$Scores, breaks=nrow(x),freq=FALSE,col="gray",main="Die Ergebnisse von deutschen OCRs für Adidas weiblich in Dichte", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Dichte")
mtext("Rot ist die Dichtfunktion",col="red")
