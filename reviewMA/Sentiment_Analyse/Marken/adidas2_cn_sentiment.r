...
pdf("adidas2_cn_sentiment.pdf")
# setwd("F:/FP_Data_Mining/R")
sink("adidas2_cn_sentiment.txt", append=TRUE)
...
Daten <-read.csv("adidas2_cn.csv", header=TRUE,fileEncoding = "UTF-8", sep = ";")
n=dim(Daten)[1]
...
# Write CSV in R
write.csv(x, file = "adidas2Ergebnis_cn.csv", na="")
h<-hist(dataSet$Scores, breaks=nrow(x),col="gray",main="Die Ergebnisse von chinesischen OCRs für Adidas weiblich in Frequenz", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Frequenz")
...
hist(dataSet$Scores, breaks=nrow(x),freq=FALSE,col="gray",main="Die Ergebnisse von chinesischen OCRs für Adidas weiblich in Dichte", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Dichte")
