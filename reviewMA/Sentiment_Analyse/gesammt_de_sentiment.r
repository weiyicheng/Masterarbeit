...
pdf("gesammt_de_sentiment.pdf")
sink("gesammt_de_sentiment.txt", append=TRUE)
...
# Write CSV in R
write.csv(x, file = "gesammtErgebnis_de.csv", na="")
h<-hist(dataSet$Scores, breaks=nrow(x),col="gray",main="Die Ergebnisse von deutschen OCRs in Frequenz", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Frequenz")
...
hist(dataSet$Scores, breaks=nrow(x),freq=FALSE,col="gray",main="Die Ergebnisse von deutschen OCRs in Dichte", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Dichte")
