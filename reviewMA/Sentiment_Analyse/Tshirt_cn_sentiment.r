...
pdf("Tshirt_cn_sentiment.pdf")
# setwd("F:/FP_Data_Mining/R")
sink("Tshirt_cn_sentiment.txt", append=TRUE)
...
Daten <-read.csv("Tshirt_cn_translated.csv", header=TRUE,fileEncoding = "UTF-8", sep = ";")
n=dim(Daten)[1]
...
# Write CSV in R
write.csv(x, file = "TshirtErgebnis_cn.csv", na="")
h<-hist(dataSet$Scores, breaks=nrow(x)/8,col="gray",main="Die Ergebnisse von chinesischen OCRs für Tshirt in Frequenz", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Frequenz")
...
hist(dataSet$Scores, breaks=nrow(x)/8,freq=FALSE,col="gray",main="Die Ergebnisse von chinesischen OCRs für Tshirt in Dichte", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Dichte")
