...
pdf("Tshirt_de_sentiment.pdf")
# setwd("F:/FP_Data_Mining/R")
sink("Tshirt_de_sentiment.txt", append=TRUE)
...
Daten <-read.csv("Tshirt_de.csv", header=TRUE,fileEncoding = "UTF-8", sep = ",")
n=dim(Daten)[1]
...
# Write CSV in R
write.csv(x, file = "TshirtErgebnis_de.csv", na="")
h<-hist(dataSet$Scores, breaks=nrow(x),col="gray",main="Die Ergebnisse von deutschen OCRs für Tshirt in Frequenz", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Frequenz")
...
hist(dataSet$Scores, breaks=nrow(x),freq=FALSE,col="gray",main="Die Ergebnisse von deutschen OCRs für Tshirt in Dichte", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Dichte")
