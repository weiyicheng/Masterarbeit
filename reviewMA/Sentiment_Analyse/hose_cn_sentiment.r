...
pdf("hose_cn_sentiment.pdf")
# setwd("F:/FP_Data_Mining/R")
sink("hose_cn_sentiment.txt", append=TRUE)
...
# Write CSV in R
write.csv(x, file = "hoseErgebnis_cn.csv", na="")
h<-hist(dataSet$Scores, breaks=nrow(x),col="gray",main="Die Ergebnisse von chinesischen OCRs für Hose in Frequenz", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Frequenz")
...
hist(dataSet$Scores, breaks=nrow(x),freq=FALSE,col="gray",main="Die Ergebnisse von chinesischen OCRs für Hose in Dichte", xlab="Polarität von OCRs durch Sentiment Analyse", ylab="Dichte")
